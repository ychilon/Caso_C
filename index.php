    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_FILES['archivos']) && is_array($_FILES['archivos']['name'])) {
            foreach ($_FILES['archivos']['name'] as $key => $nombre) {
                ...
                $extensionesProhibidas = ['php', 'xml', 'bat', 'iso', 'war', 'ear', 'jar', 'zip', 'rar', '7z', 'tar', 'gz', 'xhtml', 'json', 'svg', 'html', 'htm', 'ps1', 'exe', 'cmd', 'com', 'sh', 'py', 'cgi', 'pl', 'jsp', 'jspx', 'asp', 'aspx', 'php3', 'php4', 'phtml'
            ];
                $extension = pathinfo($nombre, PATHINFO_EXTENSION);
                
                if (in_array(strtolower($extension), $extensionesProhibidas)) {
                    $mensaje = "Error: No se permiten archivos .$extension.";
